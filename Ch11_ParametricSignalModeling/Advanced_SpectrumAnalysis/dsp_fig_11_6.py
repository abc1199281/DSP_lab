...
def arcov_compat(x, order):
    ...
    e = np.dot(a, np.dot(np.cov(x[order:], rowvar=False), a)) # Simplified variance
...
def dsp_fig_11_6():
    ...
    order = 4
    # AR model estimation using Yule-Walker method
    d, p = aryule_compat(s, order)
    # AR model estimation using Covariance method
    a, e = arcov_compat(s, order)
    
    # Frequency response of the models
