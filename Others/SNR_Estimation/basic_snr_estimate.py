...
def _my_welch_psd(signal, fs, N, window_func):
    ...
    psd_len = N // 2 # For even N, this is N/2 points, excluding Nyquist. For odd N, this is (N-1)/2 points.
    ...
    # Let's align with the Matlab code's length(tmp)/2 behavior for Pxx, which implies non-inclusive of Nyquist for even N.
    PSD = tmp[:N//2].real # Take real part as it should be real

    # Scale the PSD. Matlab's periodogram scaling can be complex.
    # The provided Matlab code scales by fs*fs (which might be due to a specific interpretation or normalization)
    # This is a key difference from standard PSD definitions which normalize by (fs * sum(window_func^2))
    # For direct translation, we will use the provided scaling.
    PSD = PSD / (fs * fs)
...
def _my_get_tone(Pxx, F):
    ...
    # Python uses 0-indexed. So idx_left >= 0. Pxx(idxLeft) <= Pxx(idxLeft+1)
    ...
    # Python: idx_right < len(Pxx). Pxx[idxRight-1] >= Pxx[idxRight]
